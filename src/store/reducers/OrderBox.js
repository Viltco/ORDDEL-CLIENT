  ...
  switch (action.type) {
    ...
    case RESEND:
      ...
      for(var i=0;i<Data.length;i++){
        ...
        updatedOrListCartItem1 = new ProductItem(Data[i]["quantity"],Data[i]["quantity"]*Data[i]["avg_price"],Data[i]["product_name"],Data[i]["product_unit"],Data[i]["avg_price"] );
        state.cardItemsArray=[{id:Data[i]["product_id"],...updatedOrListCartItem1},...state.cardItemsArray],
        ...
        state.totalAmount=parseFloat(state.totalAmount)+ parseFloat(Data[i]["quantity"]*Data[i]["avg_price"]);
      }
      return state;
    ...
    case REORDER:
      ...
      for(var i=0;i<data.length;i++){
        ...
        state.count= state.count+1;
        state.totalAmount=parseFloat(state.totalAmount)+ parseFloat(data[i]["purchased_quantity"]*data[i]["avg_price"]);
      }
      return state;
    ...
    case ADD_TO_CART:
      ...
      return {
        ...state,
        cardItemsArray:[{id:addedProduct.id,...updatedOrNewCartItem},...state.cardItemsArray],
    ...
    case DELETE_PRODUCT:
      ...
      //   return state;
      // }
      const removeIndex = state.cardItemsArray.map(function(item) { return item.id; }).indexOf(action.pid);
